...
def get_pc_info():
    ...
    for gpu in w.Win32_VideoController():
        ...
        if i > 1:
            key = f"gpu_{i}"
        result[key] = {}
        result[key]["name"] = gpu.Name
        result[key]["vram"] = gpu.AdapterRAM / 1024**3
        print(f"GPU: Name: {result[key]["name"]}, VRAM: {result[key]["vram"]} GB")
        i += 1
    ...
    for memory in w.Win32_PhysicalMemory():
        ...
        print(f"RAM: Manufacturer: {result[key]["manufacturer"]}, Speed: {result[key]["speed"]}, Capacity: {result[key]["capacity"]} GB")
        i += 1
    ...
    for disk_drive in w.Win32_DiskDrive():
        ...
        if i > 1:
            key = f"drive_{i}"
        result[key] = {}
        result[key]["model"] = disk_drive.Model
        result[key]["size"] = disk_drive.Size / 1024**3
        print(f"Drive: Model: {result[key]["model"]}, Size: {result[key]["size"]} GB")
        i += 1
    return result
...
if __name__ == "__main__":
    print("\nRunning benchmarks for image processing. Mighe take a few minutes.")
    save_results(
        [
